...
b._preferredCamera&&(b._preferredCamera=a,yield b._restartVideoStream())})}static scanImage(a,b,d,f,g=!1,k=!1,l=!1){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let m;b&&("scanRegion"in b||"qrEngine"in b||"canvas"in b||"disallowCanvasResizing"in b||"alsoTryWithoutScanRegion"in b||"byteMode"in b)?(m=b.scanRegion,d=b.qrEngine,f=b.canvas,g=b.disallowCanvasResizing||!1,k=b.alsoTryWithoutScanRegion||!1,l=b.byteMode||!1):(b||d||f||g||k)&&console.warn("You're using a deprecated api for scanImage which will be removed in the future.");
let q=!!d;try{let n,u;[d,n]=yield Promise.all([d||c.createQrEngine(),c._loadImage(a)]);[f,u]=c._drawToCanvas(n,m,f,g);let p;if(d instanceof Worker){let h=d;q||c._postWorkerMessageSync(h,"inversionMode","both");p=yield new Promise((r,z)=>{let x,v,w,y=-1;v=t=>{t.data.id===y&&(h.removeEventListener("message",v),h.removeEventListener("error",w),clearTimeout(x),null!==t.data.data?b&&r({data:l?t.data.bytes:t.data.data,cornerPoints:c._convertPoints(t.data.cornerPoints,m)}):z(c.NO_QR_CODE_FOUND))};w=t=>{h.removeEventListener("message",
v);h.removeEventListener("error",w);clearTimeout(x);z("Scanner error: "+(t?t.message||t:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",w);x=setTimeout(()=>w("timeout"),1E4);let A=u.getImageData(0,0,f.width,f.height);y=c._postWorkerMessageSync(h,"decode",A,[A.data.buffer])})}else p=yield Promise.race([new Promise((h,r)=>window.setTimeout(()=>r("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield d.detect(f);if(!h)throw c.NO_QR_CODE_FOUND;
return{data:l?(new TextEncoder).encode(h.rawValue):h.rawValue,cornerPoints:c._convertPoints(h.cornerPoints,m)}}catch(r){h=r.message||r;if(/not implemented|service unavailable/.test(h))return c._disableBarcodeDetector=!0,c.scanImage(a,{scanRegion:m,canvas:f,disallowCanvasResizing:g,alsoTryWithoutScanRegion:k,byteMode:l});throw`Scanner error: ${h}`;}}))()]);return p}catch(n){if(!m||!k)throw n;return yield c.scanImage(a,{qrEngine:d,canvas:f,disallowCanvasResizing:g,byteMode:l})}finally{q||c._postWorkerMessage(d,
"close")}})}setGrayscaleWeights(a,b,d,f=!0){c._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:b,blue:d,useIntegerApproximation:f})}setInversionMode(a){c._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a&&console.warn("Specifying a worker path is not required and not supported anymore.");let b=()=>Promise.resolve().then(function(){return e}).then(f=>f.createWorker());if(!(!c._disableBarcodeDetector&&
"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&(yield BarcodeDetector.getSupportedFormats()).includes("qr_code")))return b();let d=navigator.userAgentData;return d&&d.brands.some(({brand:f})=>/Chromium/i.test(f))&&/mac ?OS/i.test(d.platform)&&(yield d.getHighEntropyValues(["architecture","platformVersion"]).then(({architecture:f,platformVersion:g})=>/arm/i.test(f||"arm")&&13<=parseInt(g||"13")).catch(()=>!0))?b():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=
...
downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,b=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,l=a.offsetTop,m=window.getComputedStyle(a),q=m.objectFit,n=b/d,u=f/g;switch(q){case "none":var p=b;var h=d;break;case "fill":p=f;h=g;break;default:("cover"===q?n>u:n<u)?(h=g,p=h*n):(p=f,h=p/n),"scale-down"===q&&(p=Math.min(p,b),h=Math.min(h,d))}var [r,z]=m.objectPosition.split(" ").map((v,w)=>{const y=
parseFloat(v);return v.endsWith("%")?(w?g-h:f-p)*y/100:y});m=this._scanRegion.width||b;u=this._scanRegion.height||d;q=this._scanRegion.x||0;var x=this._scanRegion.y||0;n=this.$overlay.style;n.width=`${m/b*p}px`;n.height=`${u/d*h}px`;n.top=`${l+z+x/d*h}px`;d=/scaleX\(-1\)/.test(a.style.transform);n.left=`${k+(d?f-r-p:r)+(d?b-q-m:q)/b*p}px`;n.transform=a.style.transform}})}static _convertPoints(a,b){if(!b)return a;let d=b.x||0,f=b.y||0,g=b.width&&b.downScaledWidth?b.width/b.downScaledWidth:1;b=b.height&&
b.downScaledHeight?b.height/b.downScaledHeight:1;for(let k of a)k.x=k.x*g+d,k.y=k.y*b+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=a.$video.readyState)){var b=Date.now()-a._lastScanTimestamp,d=1E3/a._maxScansPerSecond;b<d&&(yield new Promise(g=>setTimeout(g,
d-b)));a._lastScanTimestamp=Date.now();try{var f=yield c.scanImage(a.$video,{scanRegion:a._scanRegion,qrEngine:a._qrEnginePromise,canvas:a.$canvas,byteMode:a._byteMode})}catch(g){if(!a._active)return;a._onDecodeError(g)}!c._disableBarcodeDetector||(yield a._qrEnginePromise)instanceof Worker||(a._qrEnginePromise=c.createQrEngine());f?(a._onDecode?a._onDecode(f):a._legacyOnDecode&&a._legacyOnDecode(f.data),a.$codeOutlineHighlight&&(clearTimeout(a._codeOutlineHighlightRemovalTimeout),a._codeOutlineHighlightRemovalTimeout=
