...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c,d,f,g=!1,k=!1){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let l;c&&("scanRegion"in c||"qrEngine"in c||"canvas"in c||"disallowCanvasResizing"in c||"alsoTryWithoutScanRegion"in c)?(l=c.scanRegion,d=c.qrEngine,f=c.canvas,g=c.disallowCanvasResizing||!1,k=c.alsoTryWithoutScanRegion||!1):(c||d||f||g||k)&&console.warn("You're using a deprecated api for scanImage which will be removed in the future.");
let p=!!d;try{let q,n;[d,q]=yield Promise.all([d||b.createQrEngine(),b._loadImage(a)]);[f,n]=b._drawToCanvas(q,l,f,g);let r;if(d instanceof Worker){let h=d;p||b._postWorkerMessageSync(h,"inversionMode","both");r=yield new Promise((m,w)=>{let x,v,t,z=-1;v=u=>{u.data.id===z&&(h.removeEventListener("message",v),h.removeEventListener("error",t),clearTimeout(x),null!==u.data.data?m({data:u.data.data,cornerPoints:b._convertPoints(u.data.cornerPoints,l)}):w(b.NO_QR_CODE_FOUND))};t=u=>{h.removeEventListener("message",
v);h.removeEventListener("error",t);clearTimeout(x);w("Scanner error: "+(u?u.message||u:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",t);x=setTimeout(()=>t("timeout"),1E4);let y=n.getImageData(0,0,f.width,f.height);z=b._postWorkerMessageSync(h,"decode",y,[y.data.buffer])})}else r=yield Promise.race([new Promise((h,m)=>window.setTimeout(()=>m("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield d.detect(f);if(!h)throw b.NO_QR_CODE_FOUND;
return{data:h.rawValue,cornerPoints:b._convertPoints(h.cornerPoints,l)}}catch(m){h=m.message||m;if(/not implemented|service unavailable/.test(h))return b._disableBarcodeDetector=!0,b.scanImage(a,{scanRegion:l,canvas:f,disallowCanvasResizing:g,alsoTryWithoutScanRegion:k});throw`Scanner error: ${h}`;}}))()]);return r}catch(q){if(!l||!k)throw q;return yield b.scanImage(a,{qrEngine:d,canvas:f,disallowCanvasResizing:g})}finally{p||b._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,
"grayscaleWeights",{red:a,green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a&&console.warn("Specifying a worker path is not required and not supported anymore.");let c=()=>Promise.resolve().then(function(){return e}).then(f=>f.createWorker());if(!(!b._disableBarcodeDetector&&"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&
(yield BarcodeDetector.getSupportedFormats()).includes("qr_code")))return c();let d=navigator.userAgentData;return d&&d.brands.some(({brand:f})=>/Chromium/i.test(f))&&/mac ?OS/i.test(d.platform)&&(yield d.getHighEntropyValues(["architecture","platformVersion"]).then(({architecture:f,platformVersion:g})=>/arm/i.test(f||"arm")&&13<=parseInt(g||"13")).catch(()=>!0))?c():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();
this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay()}_onVisibilityChange(){document.hidden?this.pause():this._active&&this.start()}_calculateScanRegion(a){let c=Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>
{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,l=a.offsetTop,p=window.getComputedStyle(a),q=p.objectFit,n=c/d,r=f/g;switch(q){case "none":var h=c;var m=d;break;case "fill":h=f;m=g;break;default:("cover"===q?n>r:n<r)?(m=g,h=m*n):(h=f,m=h/n),"scale-down"===q&&(h=Math.min(h,c),m=Math.min(m,d))}var [w,x]=p.objectPosition.split(" ").map((t,z)=>{const y=parseFloat(t);return t.endsWith("%")?(z?g-m:f-h)*y/100:y});p=this._scanRegion.width||
c;r=this._scanRegion.height||d;q=this._scanRegion.x||0;var v=this._scanRegion.y||0;n=this.$overlay.style;n.width=`${p/c*h}px`;n.height=`${r/d*m}px`;n.top=`${l+x+v/d*m}px`;d=/scaleX\(-1\)/.test(a.style.transform);n.left=`${k+(d?f-w-h:w)+(d?c-q-p:q)/c*h}px`;n.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let k of a)k.x=k.x*g+d,k.y=
k.y*c+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=a.$video.readyState)){var c=Date.now()-a._lastScanTimestamp,d=1E3/a._maxScansPerSecond;c<d&&(yield new Promise(g=>setTimeout(g,d-c)));a._lastScanTimestamp=Date.now();try{var f=yield b.scanImage(a.$video,{scanRegion:a._scanRegion,
