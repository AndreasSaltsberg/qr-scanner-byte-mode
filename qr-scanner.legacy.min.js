...
b._preferredCamera&&(b._preferredCamera=a,yield b._restartVideoStream())})}static scanImage(a,b,d,e,f=!1,g=!1,k=!1){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let l;b&&("scanRegion"in b||"qrEngine"in b||"canvas"in b||"disallowCanvasResizing"in b||"alsoTryWithoutScanRegion"in b||"byteMode"in b)?(l=b.scanRegion,d=b.qrEngine,e=b.canvas,f=b.disallowCanvasResizing||!1,g=b.alsoTryWithoutScanRegion||!1,k=b.byteMode||!1):(b||d||e||f||g)&&console.warn("You're using a deprecated api for scanImage which will be removed in the future.");
if(!d)throw Error("You have to pass a qrEngine option to the QrScanner.scanImage method");let p=!!d;try{let m,t;[d,m]=yield Promise.all([d,c._loadImage(a)]);[e,t]=c._drawToCanvas(m,l,e,f);let n;if(d instanceof Worker){let h=d;p||c._postWorkerMessageSync(h,"inversionMode","both");n=yield new Promise((q,y)=>{let w,u,v,x=-1;u=r=>{r.data.id===x&&(h.removeEventListener("message",u),h.removeEventListener("error",v),clearTimeout(w),null!==r.data.data?b&&q({data:k?r.data.bytes:r.data.data,cornerPoints:c._convertPoints(r.data.cornerPoints,
l)}):y(c.NO_QR_CODE_FOUND))};v=r=>{h.removeEventListener("message",u);h.removeEventListener("error",v);clearTimeout(w);y("Scanner error: "+(r?r.message||r:"Unknown Error"))};h.addEventListener("message",u);h.addEventListener("error",v);w=setTimeout(()=>v("timeout"),1E4);let z=t.getImageData(0,0,e.width,e.height);x=c._postWorkerMessageSync(h,"decode",z,[z.data.buffer])})}else n=yield Promise.race([new Promise((h,q)=>window.setTimeout(()=>q("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=
yield d.detect(e);if(!h)throw c.NO_QR_CODE_FOUND;return{data:k?(new TextEncoder).encode(h.rawValue):h.rawValue,cornerPoints:c._convertPoints(h.cornerPoints,l)}}catch(q){h=q.message||q;if(/not implemented|service unavailable/.test(h))return c._disableBarcodeDetector=!0,c.scanImage(a,{scanRegion:l,canvas:e,disallowCanvasResizing:f,alsoTryWithoutScanRegion:g,byteMode:k});throw`Scanner error: ${h}`;}}))()]);return n}catch(m){if(!l||!g)throw m;return yield c.scanImage(a,{qrEngine:d,canvas:e,disallowCanvasResizing:f,
byteMode:k})}finally{p||c._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,b,d,e=!0){c._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:b,blue:d,useIntegerApproximation:e})}setInversionMode(a){c._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=this._calculateScanRegion(this.$video);
...
k=a.offsetTop,l=window.getComputedStyle(a),p=l.objectFit,m=b/d,t=e/f;switch(p){case "none":var n=b;var h=d;break;case "fill":n=e;h=f;break;default:("cover"===p?m>t:m<t)?(h=f,n=h*m):(n=e,h=n/m),"scale-down"===p&&(n=Math.min(n,b),h=Math.min(h,d))}var [q,y]=l.objectPosition.split(" ").map((u,v)=>{const x=parseFloat(u);return u.endsWith("%")?(v?f-h:e-n)*x/100:x});l=this._scanRegion.width||b;t=this._scanRegion.height||d;p=this._scanRegion.x||0;var w=this._scanRegion.y||0;m=this.$overlay.style;m.width=
`${l/b*n}px`;m.height=`${t/d*h}px`;m.top=`${k+y+w/d*h}px`;d=/scaleX\(-1\)/.test(a.style.transform);m.left=`${g+(d?e-q-n:q)+(d?b-p-l:p)/b*n}px`;m.transform=a.style.transform}})}static _convertPoints(a,b){if(!b)return a;let d=b.x||0,e=b.y||0,f=b.width&&b.downScaledWidth?b.width/b.downScaledWidth:1;b=b.height&&b.downScaledHeight?b.height/b.downScaledHeight:1;for(let g of a)g.x=g.x*f+d,g.y=g.y*b+e;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in
this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=a.$video.readyState)){var b=Date.now()-a._lastScanTimestamp,d=1E3/a._maxScansPerSecond;b<d&&(yield new Promise(f=>setTimeout(f,d-b)));a._lastScanTimestamp=Date.now();try{var e=yield c.scanImage(a.$video,{scanRegion:a._scanRegion,qrEngine:a._qrEnginePromise,canvas:a.$canvas,byteMode:a._byteMode})}catch(f){if(!a._active)return;
a._onDecodeError(f)}e?(a._onDecode?a._onDecode(e):a._legacyOnDecode&&a._legacyOnDecode(e.data),a.$codeOutlineHighlight&&(clearTimeout(a._codeOutlineHighlightRemovalTimeout),a._codeOutlineHighlightRemovalTimeout=void 0,a.$codeOutlineHighlight.setAttribute("viewBox",`${a._scanRegion.x||0} `+`${a._scanRegion.y||0} `+`${a._scanRegion.width||a.$video.videoWidth} `+`${a._scanRegion.height||a.$video.videoHeight}`),a.$codeOutlineHighlight.firstElementChild.setAttribute("points",e.cornerPoints.map(({x:f,y:g})=>
`${f},${g}`).join(" ")),a.$codeOutlineHighlight.style.display="")):a.$codeOutlineHighlight&&!a._codeOutlineHighlightRemovalTimeout&&(a._codeOutlineHighlightRemovalTimeout=setTimeout(()=>a.$codeOutlineHighlight.style.display="none",100))}a._scanFrame()})})}_onDecodeError(a){a!==c.NO_QR_CODE_FOUND&&console.log(a)}_getCameraStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!navigator.mediaDevices)throw"Camera not found.";let b=/^(environment|user)$/.test(a._preferredCamera)?
"facingMode":"deviceId",d=[{width:{min:1024}},{width:{min:768}},{}],e=d.map(f=>Object.assign({},f,{[b]:{exact:a._preferredCamera}}));for(let f of[...e,...d])try{let g=yield navigator.mediaDevices.getUserMedia({video:f,audio:!1}),k=a._getFacingMode(g)||(f.facingMode?a._preferredCamera:"environment"===a._preferredCamera?"user":"environment");return{stream:g,facingMode:k}}catch(g){}throw"Camera not found.";})}_restartVideoStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let b=
...
a.videoWidth||a.width,l=b&&b.height?b.height:a.videoHeight||a.height;e||(e=b&&b.downScaledWidth?b.downScaledWidth:k,b=b&&b.downScaledHeight?b.downScaledHeight:l,d.width!==e&&(d.width=e),d.height!==b&&(d.height=b));b=d.getContext("2d",{alpha:!1});b.imageSmoothingEnabled=!1;b.drawImage(a,f,g,k,l,0,0,d.width,d.height);return[d,b]}static _loadImage(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(a instanceof Image)return yield c._awaitImageLoad(a),a;if(a instanceof HTMLVideoElement||
...
})}static _awaitImageLoad(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.complete&&0!==a.naturalWidth||(yield new Promise((b,d)=>{let e=f=>{a.removeEventListener("load",e);a.removeEventListener("error",e);f instanceof ErrorEvent?d("Image load error"):b()};a.addEventListener("load",e);a.addEventListener("error",e)}))})}static _postWorkerMessage(a,b,d,e){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){return c._postWorkerMessageSync(yield a,b,d,e)})}static _postWorkerMessageSync(a,
b,d,e){if(!(a instanceof Worker))return-1;let f=c._workerMessageId++;a.postMessage({id:f,type:b,data:d},e);return f}}c.DEFAULT_CANVAS_SIZE=400;c.NO_QR_CODE_FOUND="No QR code found";c._disableBarcodeDetector=!1;c._workerMessageId=0;return c})
